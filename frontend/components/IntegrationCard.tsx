    ...
    <div>
      ...
      <div className="relative">
        <input
          ...
          placeholder={isSet && maskedValue ? `Current: ${maskedValue}` : placeholder || `Enter ${envKey}`}
      ...
      {isSet && <p className="mt-0.5 text-xs text-emerald-700">Already set ({maskedValue})</p>}
    </div>
  );
}
...
export function IntegrationCard({ integration }: Props) {
  ...
  const isDirty = envVarKeys.some((k) => fieldValues[k] !== undefined && fieldValues[k] !== "");

  return (
    <div
      className={`rounded-xl border transition-all shadow-sm ${
        integration.configured ? "border-jarvis-300/80 bg-accent-muted/60" : "border-border bg-surface/95 backdrop-blur-sm"
      }`}
    >
      <div className="p-5">
        <div className="mb-3 flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {integration.category && <span className="text-xs text-muted">{integration.category}</span>}
            </div>
          </div>
      ...
      {configOpen && hasEnvVars && (
        <div className="border-t border-border px-5 pb-5 pt-4">
          {integration.setup_hint && <p className="mb-3 text-xs text-muted">{integration.setup_hint}</p>}
